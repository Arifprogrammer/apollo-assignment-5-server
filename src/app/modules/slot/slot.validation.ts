...
export const slotUpdateValidationSchema = z
  .object({
    room: z
      .string({
        required_error: 'A room reference is required to book a slot',
  ...
  .refine(
    schemaBody => {
      ...
      return (
        endTime > startTime &&
        Number(schemaBody.endTime.split(':')[0]) -
          Number(schemaBody.startTime.split(':')[0]) ===
          1
      )
    },
    {
      message: `Start time should be before End time! & Can't update slot for more than 1 hour duration`,
    },
  )
